...
> **What just happened?** The Workspace modal created a Git API integration for you — a one-time step that tells Snowflake which GitHub organization (`sfc-gh-kenguyen`) is an allowed source for Git-backed Workspaces.
...
CoCo will generate SQL that creates a LARGE warehouse (`LOAD_WH`), runs six `COPY INTO` statements, then drops `LOAD_WH` and restores `COMPUTE_WH`. Compare it against the expected output in the notebook — if they match, click **Allow** to run it. Optionally, copy the SQL into the notebook cell for future reference.
